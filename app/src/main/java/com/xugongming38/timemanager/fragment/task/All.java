...
public class All extends BaseFragment implements XRecyclerView.LoadingListener {
    ...
    private void initData() {
        ...
        if(!mUserName.equals(""))
        {
            allWorks =DataSupport.where("username = ?",mUserName).find(Work.class);
            if(allWorks==null)
                allWorks=new ArrayList<Work>();
            //allWorks = DataSupport.findAll(Work.class);
        }
        if(adapter!=null)
            adapter.notifyDataSetChanged();
    }
